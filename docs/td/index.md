# TD

Travaux dirigés, classés par notion.

<!-- AUTO-DOCS:START -->
## N01 — Rentrée, données, logique et automatismes

- [TD N01](TD_N01_RENTREE_DONNEES_LOGIQUE_AUTOMATISMES.pdf)

## N02 — Proportions, pourcentages et évolutions

- [TD N02](TD_N02_PROPORTIONS_POURCENTAGES_EVOLUTIONS.pdf)

## N03 — Calcul algébrique, équations et signes

- [TD N03](TD_N03_CALCUL_ALGEBRIQUE_EQUATIONS_SIGNES.pdf)

## N04 — Fonctions : registres et lectures graphiques

- [TD N04](TD_N04_FONCTIONS_REGISTRES_LECTURES_GRAPHIQUES.pdf)

## N05 — Taux de variation et fonctions monotones

- [TD N05](TD_N05_TAUX_VARIATION_FONCTIONS_MONOTONES.pdf)

## N06 — Fonctions du second degré : paraboles et formes simples

- [TD N06](TD_N06_FONCTIONS_SECOND_DEGRE_PARABOLES_FORMES_SIMPLES.pdf)

## N07 — Second degré factorisé : racines, signe et problèmes

- [TD N07](TD_N07_SECOND_DEGRE_FACTORISE.pdf)

## N08 — Suites numériques : modèles discrets

- [TD N08](TD_N08_SUITES_NUMERIQUES_MODELES_DISCRETS.pdf)

## N09 — Suites arithmétiques et géométriques

- [TD N09](TD_N09_SUITES_ARITHMETIQUES_GEOMETRIQUES.pdf)

## N10 — Modèles d'évolution, seuils, tableur et Python

- [TD N10](TD_N10_MODELES_EVOLUTION_SEUILS_TABLEUR_PYTHON.pdf)

## N11 — Nombre dérivé : sécantes, tangentes, interprétation

- [TD N11](TD_N11_NOMBRE_DERIVE_SECANTES_TANGENTES.pdf)

## N12 — Fonction dérivée et polynômes de degré au plus 3

- [TD N12](TD_N12_FONCTION_DERIVEE_POLYNOMES_DEGRE_3.pdf)

## N13 — Variations et extremums par dérivation

- [TD N13](TD_N13_VARIATIONS_EXTREMUMS_DERIVATION.pdf)

## N14 — Synthèse d'analyse et problèmes de gestion

- [TD N14](TD_N14_SYNTHESE_ANALYSE_PROBLEMES_GESTION.pdf)

## N15 — Statistiques à deux variables : nuages et point moyen

- [TD N15](TD_N15_STATISTIQUES_DEUX_VARIABLES_NUAGES_POINT_MOYEN.pdf)

## N16 — Ajustement affine, interpolation et extrapolation

- [TD N16](TD_N16_AJUSTEMENT_AFFINE_INTERPOLATION_EXTRAPOLATION.pdf)

## N17 — Probabilités conditionnelles et indépendance

- [TD N17](TD_N17_PROBABILITES_CONDITIONNELLES_INDEPENDANCE.pdf)

## N18 — Épreuves indépendantes de Bernoulli et arbres

- [TD N18](TD_N18_EPREUVES_BERNOULLI_REPETITIONS_INDEPENDANTES.pdf)

## N19 — Variables aléatoires, espérance et simulation

- [TD N19](TD_N19_VARIABLES_ALEATOIRES_ESPERANCE_SIMULATION.pdf)

## N20 — Synthèse annuelle et préparation à l'épreuve anticipée

- [TD N20](TD_N20_SYNTHESE_ANNUELLE_EPREUVE_ANTICIPEE.pdf)
<!-- AUTO-DOCS:END -->
