...
class PublicationApp:
    ...
    def deploy_github_pages(self) -> None:
        ...
        expected_source_sha = self.pushed_source_sha
        if expected_source_sha is None:
            messagebox.showerror(
                "Déploiement bloqué",
                "Déploiement bloqué : aucune source poussée par cette "
                "préparation n'est disponible.",
                parent=self.root,
            )
        ...
        confirmed = messagebox.askyesno(
            ...
            "Cette opération va créer un worktree temporaire propre depuis "
            "origin/main, vérifier qu'il correspond au commit source poussé, "
            "puis publier GitHub Pages depuis ce worktree.\n\n"
            f"Commit source attendu :\n{expected_source_sha}\n\n"
            f"Adresse publique :\n{PUBLIC_SITE_URL}\n\n"
            "Continuer ?",
            parent=self.root,
        )
        ...
        deploy_worktree = Path(
            tempfile.mkdtemp(prefix=f"{publisher.PROJECT_ROOT.name}-deploy-")
        )
        site_directory = Path(
        ...
        try:
            ...
            if origin_main != expected_source_sha:
                raise RuntimeError(
                    "Déploiement bloqué : origin/main a changé depuis le "
                    "commit + push source.\n\n"
                    f"Commit attendu : {expected_source_sha}\n"
            ...
            if deploy_head != expected_source_sha:
                raise RuntimeError(
                    "Déploiement bloqué : le worktree de déploiement ne "
                    "correspond pas au commit source attendu.\n\n"
                    f"Commit attendu : {expected_source_sha}\n"
                    f"HEAD worktree : {deploy_head}"
                )

            deploy_status = self._git_output_or_error(
                deploy_worktree, "status", "--porcelain"
            )
            if deploy_status.strip():
                raise RuntimeError(
                    "Déploiement bloqué : le worktree de déploiement n'est "
                    "pas propre.\n\n"
                    + deploy_status
                )
            ...
            if build.returncode != 0:
                raise RuntimeError(
                    "mkdocs build --strict a échoué dans le worktree de "
                    "déploiement.\n\n"
                    + build_output
                )
            ...
            command_output = format_command_output(completed)
            if completed.returncode != 0:
            ...
            self._remove_git_worktree(deploy_worktree)
        ...
        self._append_output(
            ...
            "========================\n\n"
            "✓ Worktree propre créé depuis origin/main\n"
            f"✓ Commit source déployé : {expected_source_sha}\n"
            "✓ Build strict réussi dans le worktree de déploiement\n"
            "✓ mkdocs gh-deploy exécuté depuis le worktree propre\n\n"
            f"Site public actualisé : {PUBLIC_SITE_URL}\n\n"
            "SORTIE DÉPLOIEMENT\n"
            "------------------\n"
            f"{command_output or '(aucune sortie)'}\n\n"
